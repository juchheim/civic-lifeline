...
5. **Test the Feature**
   ...
   - Complete Template → Contact → Skills → Experience → Education
   - When the Summary step loads, confirm the loader mentions the 15-second AI draft
   - Wait for the textarea to populate automatically, then (optionally) click **Regenerate summary** to request another draft
...
3. Fill in the resume builder form through the Education step
4. On the Summary step, observe the loader, verify the autogenerated draft, and optionally click "Regenerate summary"
...
curl -X POST http://localhost:3000/api/resume/summary \
  ...
  -d '{
    ...
    "topSkills": ["Customer Service", "Retail"],
    "highestEducation": {
      "labels": ["High School Diploma"],
