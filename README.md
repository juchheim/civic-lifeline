...
- SDWIS no longer honors the documented `FIPS_COUNTY_CODE` filter, so we filter responses client-side by state FIPS (matching either the `state` field or the `pwsid` prefix) and cap to the top 1,000 systems by population to keep payloads manageable.
