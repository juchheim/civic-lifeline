...
COMPLETE: 8. **Skills input lacks any accessible name and references a missing helper element**  
   - File: `apps/web/components/resume/steps/SkillsStep.tsx:1-87`.  
   - The freeform skill input now has a programmatic label and helper text tied to `skillsHelpId`, so screen readers announce it correctly.

COMPLETE: 9. **Summary textarea loses its description on mobile**  
   - File: `apps/web/components/resume/steps/SummaryStep.tsx:50-170`.  
   - The helper paragraph is now always present (no longer hidden on mobile), ensuring the textarea retains its description for screen-reader users.

COMPLETE: 10. **Experience date selectors are not associated with labels**  
    - File: `apps/web/components/resume/steps/ExperienceStep.tsx:1-277`.  
    - Each month/year `select` now has an associated label, so screen readers announce the purpose of the combo boxes instead of reporting them as unlabeled.

COMPLETE: 11. **`ActionControls` is dead code**  
