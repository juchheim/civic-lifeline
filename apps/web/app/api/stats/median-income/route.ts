...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (error instanceof CensusApiError) {
      return censusErrorResponse(error, "median-income");
    }
    console.error("[Census:median-income] Unexpected error", error);
    return NextResponse.json({ error: "Unable to load SAIPE median income." }, { status: 500 });
  }
}
