...
const log = createLogger("api/utilities/providers/electric");
const SOURCE = "HIFLD Electric Retail Service Territories";
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const cacheKey = `utilities:providers:electric:${countyFips}`;
    ...
    const [
      ...
      getElectricUtilitiesByCounty(countyFips, normalizedState),
    ]);
    ...
    const response = {
      ...
      notes: "Intersections performed with TIGERweb county geometry to scope retail territories.",
    };
    ...
    return NextResponse.json({ error: { code: "SERVER_ERROR", message: "Electric provider lookup failed." } }, { status: 500 });
  }
}
